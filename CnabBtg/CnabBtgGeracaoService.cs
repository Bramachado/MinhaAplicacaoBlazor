...
public class CnabBtgGeracaoService
{
    ...
    public async Task<CnabBtgGeracaoResultadoDto> GerarAsync(CnabBtgGerarRequest req)
    {
        ...
        var escolhidos = candidatos
            .Where(p => selecionadas.Contains(p.Chave) && (req.PermitirRegerar || !p.JaEmCnab))
            .ToList();

        if (escolhidos.Count == 0)
            throw new InvalidOperationException(req.PermitirRegerar
                ? "Nenhum pagamento selecionado para gerar."
                : "Nenhum pagamento válido selecionado (ou todos já estão em um CNAB ativo).");
        ...
        var batch = new CnabBatch
        {
            ...
            // Conferência (tela Gerenciar CNAB) — não afeta o .rem, só o histórico/auditoria.
            TipoGeracao = "TRANSFERENCIA",
            BancoCadastroId = req.BancoCadastroId,
            BancoCadastroNome = req.BancoCadastroNome,
            ValorTotalCompetencia = req.ValorTotalCompetencia,
            QuantidadeTotalCompetencia = req.QuantidadeTotalCompetencia,
            ValorTotalSelecionado = req.ValorSelecionado,
            QuantidadeSelecionada = req.QuantidadeSelecionada,
            ValorTotalGerado = resultado.ValorTotal,
            ValorTotalBoletos = req.ValorBoletos,
            QuantidadeBoletos = req.QuantidadeBoletos,
            ValorTotalInvalidos = req.ValorInvalidos
        };
