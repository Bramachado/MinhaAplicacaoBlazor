...
SELECT p.Nome, p.Cpf, p.Cargo, p.CargaHorariaSemanal, p.SalarioBase, item.TicketAlimentacao, item.SalarioFamilia, 
item.PremiacaoFixa, item.PremiacaoVariavel, item.OutrosProventos, item.DecimoTerceiro, item.FeriasUmTerco, item.TotalProventos, item.DescontoINSS, 
item.ValorFaltas, item.DescontoIR, item.TotalDescontos, item.ValorTotal, item.ValorReceberPix , 
c.NomeTitular, c.CpfCnpj, c.Forma, c.TipoPessoa, c.TipoChavePix, c.ChavePix, c.CodigoBanco, c.NomeBanco, c.Agencia, c.Conta
...
SELECT t.Nome, t.Cpf, item.Cargo, item.TotalHorasNormais, item.TotalHorasPraticas, item.ValorTotalReceber, c.NomeTitular, 
