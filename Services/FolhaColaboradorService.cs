...
public class FolhaColaboradorService
{
    private const string OrigemLancamento = "FolhaColaborador";
    private const string DescricaoLancamento = "Folha de Colaboradores";
    private const string PlanoContaPadrao = "FOLHA PAGAMENTO";

    private readonly IDbContextFactory<AppDbContext> _dbFactory;

    public FolhaColaboradorService(IDbContextFactory<AppDbContext> dbFactory)
    {
    ...
    public async Task<bool> ValidarFolhaAbertaAsync(int folhaId)
    {
        ...
        var status = await ctx.FolhasColaboradores
            .AsNoTracking()
    ...
    public async Task<decimal> RecalcularTotalAsync(int folhaId)
    {
        ...
        var folha = await ctx.FolhasColaboradores.FirstOrDefaultAsync(f => f.Id == folhaId)
            ?? throw new InvalidOperationException("Folha não encontrada.");

        folha.ValorTotal = await ctx.FolhasColaboradoresItens
